...
public final class DatabaseInitializer {
    ...
    private static void executeScript(Connection connection, String resourceName) throws Exception {
        ...
        if (input == null) throw new IllegalStateException("SQL resource not found: " + resourceName);
    ...
    private static void seedDemoData(Connection connection) throws Exception {
        if (count(connection, "users") > 0) return;
        ...
        execute(connection, "INSERT INTO bookmarked_stores (user_id, merchant_id, created_at, is_deleted) VALUES (?, ?, ?, FALSE)",
    ...
    private static void ensureDemoPin(Connection connection) throws Exception {
        if (!AppConfig.getBoolean("demo.mode", true) || count(connection, "users") == 0) return;
        String pinHash = BCrypt.hashpw("123456", BCrypt.gensalt(10));
        execute(connection, "UPDATE users SET pin_hash = ?, pin_fail_count = 0, pin_locked_until = NULL WHERE user_id = 1", pinHash);
    }
