...
public class CardRepository {
    ...
    public List<UserCard> findActiveCards(Connection connection, long userId, boolean recommendationOnly) throws SQLException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setLong(1, userId);
            try (ResultSet rs = ps.executeQuery()) { while (rs.next()) cards.add(map(rs)); }
        }
        return cards;
    }
    ...
    public UserCard findById(Connection connection, long userId, long userCardId) throws SQLException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setLong(1, userId); ps.setLong(2, userCardId);
    ...
    private UserCard map(ResultSet rs) throws SQLException {
        ...
        card.setUserCardId(rs.getLong("user_card_id")); card.setUserId(rs.getLong("user_id"));
        card.setCardId(rs.getLong("card_id")); card.setCardLast4(rs.getString("card_last4"));
        card.setPrimary(rs.getBoolean("is_primary")); card.setRecommendationEnabled(rs.getBoolean("recommendation_enabled"));
        card.setCardStatus(rs.getString("card_status")); card.setCardCompanyName(rs.getString("card_company_name"));
        card.setCardName(rs.getString("card_name")); card.setCardType(rs.getString("card_type"));
