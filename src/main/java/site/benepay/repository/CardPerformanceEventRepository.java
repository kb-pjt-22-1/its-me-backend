...
public class CardPerformanceEventRepository {
	public boolean insertIfAbsent(Connection c, long paymentId, String eventType) throws SQLException {
		try (PreparedStatement ps = c.prepareStatement(
			"INSERT INTO card_performance_events(payment_id,event_type,processed_at) VALUES(?,?,CURRENT_TIMESTAMP)")) {
	...
	private boolean isDuplicate(SQLException e) {
		...
		return "23505".equals(state) || e.getErrorCode() == 1062 || String.valueOf(e.getMessage())
			.toLowerCase()
			.contains("unique");
	}
}
