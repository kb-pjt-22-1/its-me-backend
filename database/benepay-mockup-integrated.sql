...
-- Full payment features extension
...
CREATE TABLE IF NOT EXISTS payment_delivery_outbox (
 outbox_id BIGINT AUTO_INCREMENT PRIMARY KEY, payment_id BIGINT NOT NULL, event_type VARCHAR(20) NOT NULL, payload_json TEXT NOT NULL,
 delivery_status VARCHAR(20) NOT NULL DEFAULT 'PENDING', retry_count INT NOT NULL DEFAULT 0, next_retry_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
 last_error VARCHAR(500), created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, delivered_at DATETIME NULL,
 UNIQUE KEY uk_payment_outbox(payment_id,event_type), FOREIGN KEY(payment_id) REFERENCES payments(payment_id), INDEX idx_outbox_retry(delivery_status,next_retry_at)
) ENGINE=InnoDB;
CREATE TABLE IF NOT EXISTS card_performance_events (payment_id BIGINT NOT NULL,event_type VARCHAR(20) NOT NULL,processed_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(payment_id,event_type),FOREIGN KEY(payment_id) REFERENCES payments(payment_id)) ENGINE=InnoDB;
